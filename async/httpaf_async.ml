...
module Server = struct
  let start_read_write_loops
    ...
    let rec writer_thread () =
      ...
      | `Close _ ->
        ...
        Deferred.don't_wait_for (close_write socket >>| Ivar.fill write_complete)
    in
  ...
  module SSL = struct
    let create_connection_handler
      ...
      start_read_write_loops ~config ~readf ~writev ~socket ~close_read ~close_write conn
    ;;
  end
end
...
module Client = struct
  let start_read_write_loops
    ...
    don't_wait_for
      (Deferred.all_unit [ Ivar.read read_complete; Ivar.read write_complete ]
       >>| fun () -> if not (Fd.is_closed fd) then don't_wait_for (Fd.close fd))
  ;;

  let request ?(config = Config.default) socket request ~error_handler ~response_handler =
    let request_body, conn =
      Client_connection.request request ~error_handler ~response_handler
    in
    start_read_write_loops ~config ~socket conn;
    request_body
  ;;
  ...
  module SSL = struct
    let request
      ...
      >>| (fun ssl ->
        ...
        let readf = Ssl_io.readf ssl_reader in
        let writev = Ssl_io.writev ssl_writer in
        start_read_write_loops ~config ~readf ~writev ~socket conn)
